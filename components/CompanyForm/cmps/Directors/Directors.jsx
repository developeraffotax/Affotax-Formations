...
const Directors = ({ addresss, setAddress, continueBtnHandler }) => {
...
useEffect(() => {
 ...
 if (residential_address_isSameAsService) {
      ...
      setValue('residential_address_name_or_number', getValues('service_address_name_or_number'))
      setValue('residential_address_street', getValues('service_address_street'))
      setValue('residential_address_locality', getValues('service_address_locality'))
      setValue('residential_address_town', getValues('service_address_town'))
      setValue('residential_address_county', getValues('service_address_county'))
      setValue('residential_address_postcode', getValues('service_address_postcode'))
      setValue('residential_address_country', getValues('service_address_country'))
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="font-poppins w-full flex flex-col justify-start items-start gap-6 " >
      {/* <div className="w-full border p-5   ">
        <div className="w-full text-start px-5  bg-cyan-500   text-white py-2 my-2  shadow-md text-xl ">
          Prefill Address
        </div>

        <div className="w-full flex flex-row justify-between items-center gap-4 py-2"> <label className="text-base pl-2">Prefill</label> <Controller control={control} name="pre_filled" render={({ field }) => ( <Select {...field}  size="sm" > {pre_filled_address_array.map((address) => ( <SelectItem key={address.country} >{`${address.name_or_number}, ${address.street}, ${address.town}, ${address.country}, ${address.postcode}`}</SelectItem> ))} </Select> )} /> </div> 
      </div> */}
      ...
      <div className="w-full flex flex-col justify-start items-start gap-2 border     ">
        ...
        {/* <div className="w-full"> {" "} <Controller control={control} name="person_country_of_residence" render={({ field }) => ( <Input {...field}  size="sm"  errorMessage={errors?.person_country_of_residence?.message} isInvalid={errors?.person_country_of_residence ? true : false} key="person_country_of_residence" classNames={{ mainWrapper: "w-full ", label: "w-[40%] text-base ", }} label="Country of residence*	" labelPlacement="outside-left" type="text" /> )} /> </div> */}

        {/* onSelectionChange={(key) =>  {key && clearErrors('country'); }} */}
        <div className=" w-full flex flex-row justify-between items-center gap-4 ">
          <label className={`text-base ${errors?.country ? "text-rose-500" : ""} ml-2`} > Select Country* </label>
          <Controller control={control} name="person_country_of_residence" render={({ field }) => ( <Select {...field}  classNames={{base: "w-[60%]"}}  size="sm"  variant="bordered" onChange={(e) => field.onChange(e)} selectedKeys={[field.value]} > {COUNTRIES.map((country, index) => ( <SelectItem key={country}>{country}</SelectItem> ))} </Select> )} />
        </div> 
      ...
      <div className="w-full  ">
       ...
       <div className="w-full text-start    bg-cyan-500   text-white py-2 px-8  shadow-md text-xl ">
        ...
        {/* <div className="w-full text-start px-5      text-black py-2 my-2  shadow-md text-xl ">
          Prefill Address
        </div> */}
        ...
        {/* <div className="w-full"> {" "} <Controller control={control} name="service_address_country" render={({ field }) => ( <Input {...field}  size="sm"  errorMessage={errors?.service_address_country?.message} isInvalid={errors?.service_address_country ? true : false} key="service_address_country" classNames={{ mainWrapper: "w-full ", label: "w-[40%] text-base ", }} label="Postcode*	" labelPlacement="outside-left" type="text" /> )} /> </div> */}

        {/* onSelectionChange={(key) =>  {key && clearErrors('country'); }} */}
        <div className=" w-full flex flex-row justify-between items-center gap-4  ">
          <label className={`text-base ${errors?.service_address_country ? "text-rose-500" : ""}   ml-2`} > Select Country* </label>
          <Controller control={control} name="service_address_country" render={({ field }) => ( <Select {...field}  classNames={{base: "w-[60%]"}} size="sm"  variant="bordered" onChange={(e) => field.onChange(e)} selectedKeys={[field.value]} > {COUNTRIES.map((country, index) => ( <SelectItem key={country}>{country}</SelectItem> ))} </Select> )} />
        </div>
      ...
      <div className="w-full  ">
       ...
       <div className="w-full text-start  bg-cyan-500   text-white py-2 px-8  shadow-md text-xl ">
        ...
        <div className="w-full border p-5 flex flex-row justify-start items-center gap-8  px-8 ">
            ...
            <div className="w-full pl-2"> {" "} <Controller control={control} name="residential_address_isSameAsService" render={({ field }) => ( <Checkbox  {...field}  size="md" color="success" errorMessage={errors?.residential_address_isSameAsService?.message} isInvalid={errors?.residential_address_isSameAsService ? true : false} key="residential_address_isSameAsService" classNames={{ base: "w-full flex flex-row-reverse gap-5   "  , label: " text-lg" }}    > Same as service address </Checkbox> )} /> </div>

        {/* <div className="w-full flex flex-row justify-start items-center gap-4 py-2">
            </div>  */}
        ...
        {/* <div className="w-full"> {" "} <Controller control={control} name="residential_address_country" render={({ field }) => ( <Input isDisabled={isResidentialDivDisable}  {...field}  size="sm"  errorMessage={errors?.residential_address_country?.message} isInvalid={errors?.residential_address_country ? true : false} key="residential_address_country" classNames={{ mainWrapper: "w-full ", label: "w-[40%] text-base ", }} label="Postcode*	" labelPlacement="outside-left" type="text" /> )} /> </div> */}

        {/* onSelectionChange={(key) =>  {key && clearErrors('country'); }} */}
        <div className=" w-full flex flex-row justify-between items-center gap-4 ">
          <label className={`text-base ${errors?.residential_address_country ? "text-rose-500" : ""} ml-2`} > Select Country* </label>
          <Controller control={control} name="residential_address_country" render={({ field }) => ( <Select  isDisabled={isResidentialDivDisable} {...field}  classNames={{base: "w-[60%]"}}  size="sm"  variant="bordered" onChange={(e) => field.onChange(e)} selectedKeys={[field.value]} > {COUNTRIES.map((country, index) => ( <SelectItem key={country}>{country}</SelectItem> ))} </Select> )} />
        </div>
      ...
      <div className="w-full flex flex-col justify-center items-center gap-4  ">
        ...
        {errorMsg && (
          <p className="w-full text-center   text-red-500 mt-2"> {errorMsg} </p>
        )}
      </div>
    </form>
  );
