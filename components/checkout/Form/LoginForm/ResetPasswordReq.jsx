...
const ResetPasswordReq = ({isFpModal, setIsFpModal}) => {
  ...
  return (
    (<Modal backdrop="blur" isOpen={isFpModal} onClose={() => setIsFpModal(false)} isDismissable={false}>
      <ModalContent>
        ...
        {(onClose) => (
          ...
          <>
            ...
            {
              !isSuccess ? 
              <form  onSubmit={handleSubmitForm2(formSubmitHandler)} className="w-full flex flex-col justify-center items-center gap-8 pb-8 ">
            ...
            <div className="w-full ">
             ...
             <Button type="submit" color="warning" className="w-[40%] px-4 font-poppins  " >
                {isLoading ? <Spinner size="sm" color="white" /> : "Reset Password"}
              </Button>
             </div>
            </form>
            : <div key="success" className="w-full flex items-center my-3 font-poppins"> <Alert color="success" title={`Password reset link has been send to your email! Kindly check your email`} /> </div>
            }
...

            {
              ...
              <p className="w-full text-sm font-poppins text-red-500 ">{error}</p>
