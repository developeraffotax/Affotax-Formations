...
const Steps = () => {
  return (
    <section  className="w-full  bg-gradient-to-tl from-gray-100 to-orange-50 pt-0 pb-16 ">
      ...
      <div className="w-full px-4 py-16 max-xl:py-0  ">
      <div className="max-w-xl mb-10 md:mx-auto sm:text-center lg:max-w-2xl md:mb-12">
        <h2 className="max-w-lg mb-6 font-poppins text-4xl max-xl:text-3xl  max-xl:text-start font-bold max-xl:font-semibold     text-gray-900    md:mx-auto"> <span className="relative inline-block"> {" "} <svg viewBox="0 0 52 24" fill="currentColor" className="absolute top-0 left-0 z-0 hidden w-32 -mt-8 -ml-20 text-blue-gray-100 lg:w-32 lg:-ml-28 lg:-mt-10 xl:block" > {" "} <defs> {" "} <pattern id="f51618fb-0edb-4bcb-b35a-ffc770941286" x="0" y="0" width=".135" height=".30" > {" "} <circle cx="1" cy="1" r=".7" />{" "} </pattern>{" "} </defs>{" "} <rect fill="url(#f51618fb-0edb-4bcb-b35a-ffc770941286)" width="52" height="24" />{" "} </svg> </span>Company formation in 4 simple steps </h2>
        <p className="text-base text-gray-700 md:text-lg font-Inter "> Our easy company registration whole process can be completed within 15mint. </p>
      </div>

      <div className="grid gap-8 row-gap-0 xl:grid-cols-4  grid-cols-1 ">
        {
          LIST.map((el, i) => {
            return (
              <div key={el.id} className="relative text-center">
            
                  <div className="flex shadow-[0px_4px_6px_0px_rgba(0,_0,_0,_0.2)] items-center justify-center text-5xl text-primary mx-auto mb-4 rounded-full bg-white/30 backdrop-blur-md   w-32 h-32">
                      {el.icon}
                  </div>
                  ...
                  {
                    !(i === LIST.length - 1) && <div className="top-0 right-0 flex items-center justify-center h-36 max-xl:h-20 lg:-mr-8 lg:absolute">
